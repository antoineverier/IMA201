...
#%% TP FILTRAGE RESTAURATION - ANTOINE VERIER

im=skio.imread('images/lena.tif')
viewimage(im)
viewimage(rotation(im,45))
viewimage(rotation(im,45, ech=1))

# PARTIE 2 %%

im_ppv=skio.imread('images/lena.tif')
im_bil = im_ppv
viewimage(im)

for i in range(8):
    im_ppv = rotation(im_ppv,45, clip = True)
    im_bil = rotation(im_bil,45,ech=1, clip = True)
...
im_ppv=skio.imread('images/lena.tif')
viewimage(im)
viewimage(rotation(im_ppv,45,1/2))
viewimage(rotation(im_ppv,45,1/2, ech=1))

# PARTIE 3 %%
im = skio.imread('images/carre_orig.tif')
viewimage(im)
im = noise(im,1)
im = filtre_lineaire(im, get_gau_ker(1))
viewimage(im)
# %%
# PARTIE 3 %%
...
im_noise = noise(im,3)
viewimage(im_noise)
im_filtre = filtre_lineaire(im_noise, get_gau_ker(1))
viewimage(im_filtre)
# %%
var_image(im,0,0,10,10)
var_image(im_noise,0,0,10,10)
var_image(im_filtre,0,0,10,10)
...
viewimage(im_noise)
im_filtrelin = filtre_lineaire(im_noise, get_gau_ker(2))
viewimage(im_filtrelin)
im_filtremed = median_filter(im_noise)
viewimage(im_filtremed)

# %%
var_image(im_filtrelin,100,100,200,250)
...
viewimage(im)
im_filtrelin = filtre_lineaire(im, get_gau_ker(2))
viewimage(im_filtrelin)
im_filtremed = median_filter(im)
viewimage(im_filtremed)
# %%
im = skio.imread('images/carre_orig.tif')
viewimage(im)
im_filtrelin = filtre_lineaire(im, get_gau_ker(1))
viewimage(im_filtrelin)
im_filtremed = median_filter(im)
viewimage(im_filtremed)
# %%
...
im = skio.imread('images/lena.tif')
imlin = filtre_lineaire(im,get_gau_ker(1))
iminv = filtre_inverse(imlin, get_gau_ker(1))
...
im = skio.imread('images/lena.tif')
imlin = filtre_lineaire(im,get_gau_ker(1))
imnoise = noise(imlin,1)
iminv = filtre_inverse(imnoise, get_gau_ker(1))
...
# %%
viewimage(filtre_lineaire(imnoise, get_cst_ker(1)))
# %% 
